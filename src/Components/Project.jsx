...
export default function Project({ data }) {
  ...
  const handleShowAbout = (e) => {
    setAboutId(e.target?.parentNode?.nextSibling?.firstChild?.id);
    // setAboutId(e.target.id)
    //onClick={(e) => handleShowAbout(e)}
    // console.log(e.target?.parentNode?.nextSibling?.firstChild?.id)
  };
  ...
  return (
    <>
      {data.map((el) => (
        <Cont  key={uuid()} >
          <ImgCont onMouseEnter={(e) => handleShowAbout(e)}>
            <img src={el.img} alt="" />
          </ImgCont>
          {el.id === aboutId ? (
            <DetailData>
